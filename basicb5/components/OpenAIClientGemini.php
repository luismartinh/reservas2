...
class OpenAIClientGemini extends Component
{
    private $apiKey;
     ...
     * @param \app\models\base\Usuario $user
    ...
    public function askQuestion($question,$user)
    {
        ...
        $systemInstruction_text.="controladores:" . json_encode($this->getControllersData()) . "\n";
        $systemInstruction_text.="modelos:" . json_encode($this->getModelsData()) . "\n";
        ...
        if (isset($responseData['candidates'][0]['content']['parts'][0]['text'])) {
            ...
            // Eliminar ```html ``` y ``` en la respuesta
            $respuesta = preg_replace('/```html|```/', '', $responseData['candidates'][0]['content']['parts'][0]['text']);            
    ...
    {
        // Aquí puedes cargar los datos de los modelos desde tu archivo JSON
        $filePath = Yii::getAlias('@app/data/export_users.json');
        return json_decode(file_get_contents($filePath), true);
    }
    ...
    {
        // Aquí puedes cargar los datos de los modelos desde tu archivo JSON basicb5/data/gemini/systemInstruction.txt
        $filePath = Yii::getAlias('@app/data/gemini/systemInstruction.txt');
        return file_get_contents($filePath);
    }



    private function getUsersAndAccess()
    {
        $db = Yii::$app->db;
        $sql = "
        ...
        file_put_contents(__DIR__ . '/debug_openai_gemini_get_users.json', $json);
